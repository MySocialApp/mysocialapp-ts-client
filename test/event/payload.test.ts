...
describe("addMessage account", () => {
    it("user creation api", async () => {
        try {
            ...
            console.info('payload', event.toJson());
            let eventCreated = await session1.event.create(event);
            console.info('event', eventCreated);
            expect(eventCreated.id.length).toBeGreaterThan(5);
            expect(eventCreated.payload['test']).toBeTruthy();
            expect(eventCreated.getPayloadValue('test')).toBeTruthy();
