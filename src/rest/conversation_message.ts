...
export class RestConversationMessage extends Rest {
    ...
    async list(id: string, page: number, size?: number): ConversationMessage[] {
    ...
    async create(id: string, message: ConversationMessage): ConversationMessage {
        let path = Rest.params("/conversation/{id}/message", {id: id});
        return this.conf.post(new ConversationMessage(), path, message) as Promise<ConversationMessage>;
    }

    async postFile(id: string, message: ConversationMessagePost): ConversationMessage {
        let fd = new FormData();
