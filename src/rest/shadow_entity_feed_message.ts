...
export class RestShadowEntityFeedMessage extends Rest {
    ...
    async list(id: string, page: number, size: number = 10): Feed[] {
    ...
    async create(id: string, message: TextWallMessage): Feed {
        return this.conf.post(new Feed(), Rest.params("shadow/entity/{id}/wall/message", {id: id}), message) as Promise<Feed>;
    }

    async update(id: string, messageId: string, message: TextWallMessage): Feed {
    ...
    async delete(id: string, messageId: string): void {
