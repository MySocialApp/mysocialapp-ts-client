...
class Conversation extends base_1.Base {
    ...
    sendMessage(message) {
        if (!message.isMultipart) {
            return (new conversation_message_1.RestConversationMessage(this.conf)).post(this.id_str, message.getConversationMessage());
        }
        return (new conversation_message_1.RestConversationMessage(this.conf)).postFile(this.id_str, message);
    }
}
